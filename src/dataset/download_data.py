...
def make_path_and_url(clip_id):
    """
    For a Clip ID return the Path to download it locally and the url
        where the file download is located
    :param clip_id:
    :return file_path, url:
    """
    file_path = os.path.join('Raw_Capuchinbird_Clips',
                             f'XC{clip_id} - Capuchinbird - Perissocephalus tricolor.mp3')
    url = f'https://xeno-canto.org/{clip_id}/download'
    return file_path, url

def download_and_unzip_sounds(id_url_list):
    """
    Downloads and Unzips into the correct folder all the Raw Not Capuchin Clips
...
def parse_capuchinbird_clips(clip_tuple):
    ...
    ms_to_seconds = 1000
    mp3_filename = os.path.join('Raw_Capuchinbird_Clips',
                                f"XC{clip_id} - Capuchinbird - Perissocephalus tricolor.mp3")
    ...
    for start, end in starts_and_ends:
        ...
        sub_clip.export(os.path.join('Parsed_Capuchinbird_Clips',
                                     f'{sub_clip_name}.wav'), format='wav')
        count += 1

def parse_not_capuchinbird_clips(raw_file_name):
    raw_audio_folder_path = 'Raw_Not_Capuchinbird_Clips'
    selected_clip_folder_path = 'Parsed_Not_Capuchinbird_Clips'
    ...
    if num_clips > 0:
        ...
        for clip in array_vals:
            ...
            clip.export(os.path.join(selected_clip_folder_path, f'{parsed_file_name}-{count}.wav'), format='wav')
            count += 1
    else:
        sound.export(os.path.join(selected_clip_folder_path, f"{parsed_file_name}-0.wav"), format='wav')

def df_to_list_of_call_tuples(df):
    """
    Extracts a list of tuples from the provided Parsing_Single
    :param df:
    :return output:
    """
    output = list()
...
def download_dataset():
    ...
    list_dir = ['Raw_Not_Capuchinbird_Clips',
                'Raw_Capuchinbird_Clips',
